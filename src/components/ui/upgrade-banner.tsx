    ...
    <div className={cn("mx-auto flex items-center justify-center", className)}>
      <AnimatePresence>
        ...
        >
          ...
          >
            <SettingsFilled className="text-azimute-blue dark:text-azimute-blue-soft" />
          </motion.div>
          <motion.div
            initial="hidden"
            animate={isHovered ? "visible" : "hidden"}
          ...
          <div className="relative flex h-[35px] items-center gap-1 rounded-sm border border-azimute-blue/20 bg-azimute-blue-soft/5 pl-4 pr-4 text-sm dark:border-white/10 dark:bg-white/5 transition-colors">
            <Wrapper
              href={href}
              className="focus-visible:shadow-focus-ring rounded-xs my-[-1px] cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-azimute-charcoal no-underline decoration-azimute-blue/30 underline-offset-[5px] outline-none hover:text-azimute-blue hover:decoration-azimute-blue/60 focus-visible:!shadow-[#008FFF] dark:text-[#FAFAF7] dark:decoration-white/20 dark:hover:text-azimute-blue-soft dark:hover:decoration-white/40 flex items-center gap-2"
            ...
            {description && (
              <span className="text-[0.8125rem] text-azimute-blue dark:text-azimute-blue-soft">
                {description}
              </span>
            )}
            {onClose && (
              <button 
                onClick={onClose}
                className="m-0 flex h-6 !w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-azimute-blue hover:bg-azimute-blue/10 dark:hover:bg-white/10"
              >
                <X size={16} className="text-azimute-blue dark:text-azimute-blue-soft" />
              </button>
            )}
          </div>
        </motion.div>
